...
CCBuddyは、macOSのメニューバーに常駐するElectronアプリ。
...
| データ | 保存先 |
|-------|--------|
| プロジェクト名 | `~/Library/Application Support/ccbuddy/project-names.json` |
...
- **自動承認済みセッション**: Claude Code で「Yes, allow all」を選択した操作は承認プロンプトが出ないため CCBuddy では検知不可

## 将来構想 (CLI ラッパー方式)

`src/cli/` および `src/shared/protocol.ts` に、node-pty を使った CLI ラッパー方式のコードが残されている。
この方式では `ccbuddy claude ...` でラッパー経由で Claude Code を起動し、PTY を直接制御して承認を送信する。
